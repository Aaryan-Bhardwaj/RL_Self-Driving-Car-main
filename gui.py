...
speedFile = open('avgVel.txt', 'w')
brakeFile = open('brakeCount.txt', 'w')
...
while episode_count < config.MAX_EPISODE + config.TESTING_EPISODE * 3:
    ...
    while True: # frame < config.MAX_FRAME_COUNT:
        ...
        if len(cars) < PREDEFINED_MAX_CAR and np.random.standard_normal(1)[0] >= 0:
            ...
            if len(position) > 0:
                # Back
                if map_position:
                    new_car_speed = np.random.randint(30, 90)
                    new_car_y = 1010
                    new_car_lane = int(np.random.choice(position, 1))
                else:
                    new_car_speed = np.random.randint(30, 60)
                    new_car_y = -100
                    new_car_lane = int(np.random.choice(position, 1))
                # Decide lanes
    ...
    rewardsFile.write("%s %s\n" % (episode_count, score.score))
    deep_traffic_agent.model.log_hard_brake_count(subject_car.hard_brake_count)
    brakeFile.write("%s %s\n" % (episode_count, subject_car.hard_brake_count))
