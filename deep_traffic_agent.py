...
class DeepTrafficAgent:
    ...
    def remember(self, reward, next_state, end_episode=False, is_training=True):
        ...
        self.count_states = self.model.get_count_states()
        print(self.model.get_count_states(), LEARN_START, len(self.memory), end='\r')
        print(is_training==True and self.model.get_count_states() >= LEARN_START and len(self.memory) >= LEARN_START, end='\r')

        if (is_training==True and self.model.get_count_states() >= LEARN_START and len(self.memory) >= LEARN_START):
